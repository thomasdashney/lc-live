...
function deserializePrograms (programs) {
  return mapValues(programs, (serialized, output) => {
    const { programFormat } = devicesConfig.outputs[output]

    if (programFormat) {
      return programDeserializers[programFormat](serialized)
    } else {
...
const songsByKey = mapValues(songsConfig, (config, key) => {
  const sceneConfigs = config.scenes || []
 ...
 */
const matrix = {
  nord: ['prophet', 'juno', 'sampler']
}

const performance = new Performance(setlist, outputs, inputs, matrix)

/**
 * Configure footswitch handlers
 */

inputs.footswitch.on('message', message => {
  // TODO
})

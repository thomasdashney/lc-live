...
describe('start', () => {
  ...
  it('loads the first song/scene', () => {
    performance.load = jest.fn()
    performance.start()
    expect(performance.load).toBeCalledWith(0, 0)
  })
})

describe('load', () => {
  it('throws an error if no song exists for the index', () => {
    expect(() => performance.load(2, 0)).toThrow()
    expect(() => performance.load(-1, 0)).toThrow()
  })

  it('throws an error if no song exists for the index', () => {
    expect(() => performance.load(0, -1)).toThrow()
    expect(() => performance.load(0, 2)).toThrow()
  })

  it('updates current song/scene indexes', () => {
    performance.load(1, 0)
    expect(performance.currentSongIndex).toEqual(1)
    expect(performance.currentSceneIndex).toEqual(0)
  })

  it('ignores the scene index if the song has no scenes', () => {
    const setlist = new Setlist([
      new Song('some song', [], {})
    ])
    performance = new Performance(setlist, outputs, inputs, {})
    performance.load(0, 0)
    expect(performance.currentSongIndex).toEqual(0)
    expect(performance.currentSceneIndex).toEqual(null)
  })
...
describe('navigating between songs/scenes', () => {
  ...
  describe('incrementScene', () => {
    it('loads next scene of the current song', () => {
      ...
      expect(performance.load).toBeCalledWith(0, 1)
    })

    it('loads the next song if in the last scene of a song', () => {
      setIndexes(0, 1)
      performance.incrementScene()
      expect(performance.load).toBeCalledWith(1, 0)
    })

    it('does nothing when at the last scene of the set', () => {
      setIndexes(1, 1)
      performance.incrementScene()
      expect(performance.load).not.toBeCalled()
    })
  })

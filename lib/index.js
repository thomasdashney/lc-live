...
const messageHandlers = {
  ...
  loadedScene: ({ songIndex, sceneIndex }) => {
    ...
    if (songIndex !== currentSongIndex) {
      const sceneLog = chalk.grey(`${song.scenes.map(scene => scene.name).join(' => ')}`)
      logger.info(`${chalk.green(song.name)} ${sceneLog}`)
    }
    const sceneIndexSummary = chalk.grey(`(${sceneIndex + 1}/${song.scenes.length})`)
    logger.info(`  ${chalk.yellow(scene.name)} ${sceneIndexSummary}`)

...
const { prophet, nord, juno } = require('../output-devices')
...
describe('juno deserializeProgram', () => {
  it('deserializes juno-formatted patch numbers', () => {
    expect(juno.deserializeProgram('1:1')).toEqual({ program: 0 })
    expect(juno.deserializeProgram('1:8')).toEqual({ program: 7 })
    expect(juno.deserializeProgram('2:1')).toEqual({ program: 8 })
  })
})
